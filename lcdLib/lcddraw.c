...
/** 5x7 font - this function draws background pixels
 *  Adapted from RobG's EduKit, modification of previous method, modified to read a different font
 */
/*void drawPacChar(u_char rcol, u_char rrow, char c, 
     u_int fgColorBGR, u_int bgColorBGR) 
{
  u_char col = 0;
  u_char row = 0;
...
while (row < 8) {
    while (col < 5) {
      u_int colorBGR = (pacFont_5x7[oc][col] & bit) ? fgColorBGR : bgColorBGR;
      lcd_writeColor(colorBGR);
      col++;
    }
    col = 0;
 ...
 */
/*
void drawPacmanFont(u_char col, u_char row, char *string,
		u_int fgColorBGR, u_int bgColorBGR)
{
  u_char cols = col;
  while (*string) {
    drawPacChar(cols, row, *string++, fgColorBGR, bgColorBGR);
    cols += 6;
  }
  }*/
